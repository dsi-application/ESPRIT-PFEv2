...
export default class EtatExpertiseGlobal extends Component {
  constructor(props) {
    super(props);
    ...
    this.handleDownloadxpertiseStatus = this.handleDownloadxpertiseStatus.bind(this);
    ...
    this.loadAcademicExperts();

  }

  handleDownloadxpertiseStatus()
  {
    this.setState({showLoadingExcelIcon: true})

    ResponsableServiceStageService.downloadAllExpertiseStatusByYear(this.state.selectedYear).then(
        (response) => {
  ...
  loadAcademicExperts()
  {
    ...
    PedagogicalCoordinatorService.gotListAcademicExpertsByYear(this.state.selectedYear).then(
        (response) => {
  ...
  showAffectedStudentsList(idEns, nomEns)
  {
    ...
    AuthService.findAllAffectedStudentsToAExpByYear(idEns, this.state.selectedYear).then(
        (response) => {
  ...
  onChangeSelectCEP = (selectedCompOption) => {
    ...
    this.state.loadAcademicExperts = true;
    ...
    console.log('----------3.2-------------> 21.09.22: ' + this.state.loadAcademicExperts);
  ...
  */
    ...
    PedagogicalCoordinatorService.gotListAcademicExpertsByYear(selectedCompOption.label).then(
        (response) => {
          this.setState({
            academicExperts: []
          });

          let stuList = this.state.academicExperts.slice();
                ...
                <CCol md="1">
                  {
                    !showLoadingExcelIcon &&
                    <CButton shape="pill"
                             onClick={() => this.handleDownloadxpertiseStatus()}>
                      <CTooltip content="Télécharger l'État Expertise Détaillé"
                                placement="top">
                          ...
                          <CRow>
                            <CCol md="10" class="bg-success py-2">
                              <span style={{color: "#cc0000", fontSize: "14px", fontWeight: "bold"}}>
                                Liste des Étudiants pour l'Année Universitaire &nbsp;&nbsp;<span
                                  className="clignoteRed">{selectedYear} - {Number(selectedYear) + 1}</span>
                              </span>
                            </CCol>
                          </CRow>


                          <CDataTable items={academicExperts}
                                              ...
                                              <center>
                                                {
                                                  ...
                                                  <CButton shape="pill"
                                                           ...
                                                           aria-expanded="true">
                                                    <CTooltip
                                                        content="Aucune expertise n’est affectée à cet enseignant"
                                                        placement="top">
