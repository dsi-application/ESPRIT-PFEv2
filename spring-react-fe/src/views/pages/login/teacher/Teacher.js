  ...
  },
  {
    ...
    content: () => (
        <Text>
          ...
          <a href="https://pfe.esprit.tn/forgotTeacherPassword" target="_blank" rel="linkFP" style={{color:'blue'}}>Mot de Passe oublié ?</a>
        </Text>
    )
  },
    {
...
export default class Teacher extends Component {
  ...
  handleLogin(e) {
    ...
    if (this.checkBtn.context._errors.length === 0) {
      // console.log("--------------------> TRY");
      AuthService.loginTeacher(this.state.username, this.state.password).then(
        (response) => {
          ...
          if (!response.data.accessToken) {
            ...
            if (response.data)
              this.props.history.push("/secureTeacherPassword");
          }

          if (response.data.accessToken) {
            // console.log("-------------0811------------> ** Sign-In 2: " , JSON.stringify(response.data));
            sessionStorage.setItem("teacher", JSON.stringify(response.data));
            //console.log("-------------0811----j--------> ** Sign-In 2: " , JSON.parse(sessionStorage.getItem("teacher")).id);

            //sessionStorage.setItem("PIK", JSON.parse(sessionStorage.getItem("teacher")).id);
            //sessionStorage.setItem("teaENS", JSON.parse(sessionStorage.getItem("teacher")).idEns);
                  ...
                  <CCard className="text-white bg-light py-4">
                    <CCardBody className="text-center">
                      ...
                      <p className="userSpace">
                        <Wave text="Espace Encadrant Académique" effect="stretch" effectChange={1.2} />
                      </p>
